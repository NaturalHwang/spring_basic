...
// basicApplication보다 다른 소스코드가 상위 디렉토리에 있으면 안됨.
@Controller
@RequestMapping("/hello")  //클래스 차원에 url 매핑 시에 RequestMapping 사용
//@RestController // Controller + 각 메서드마다 @ResponseBody
// 해당 클래스가 컨트롤러임을 명시(사용자의 요청을 처리하고 응답하는 편의 기능 내장)
public class HelloController {

//    case1. 사용자가 서버에게 화면 요청(get)
//    case2. @ResponseBody가 붙을 경우 단순 String 데이터 return(get)
    @GetMapping("")
//    getmapping을 통해 get요청을 처리하고 url패턴을 명시
//    @ResponseBody
//    responsebody를 사용하면 화면이 아닌 데이터를 return이 스트링이면 스프링은
//    만약 여기서 responsebody가 없으면 스프링은 templates폴더 밑에 helloworld.html 화면을 찾아 리턴
...
//    public String helloJson() throws JsonProcessingException {
        ...
        hello.setName("hi");
        hello.setEmail("hello@gmail.com");
//        ObjectMapper objectMapper = new ObjectMapper();
//        return objectMapper.writeValueAsString(hello);
        return hello;
    }

//    case4. 사용자가 json데이터를 요청하되, parameter 형식으로 특정개체 요청
//    get요청 중에 특정 데이터를 요청
    @GetMapping("/param1")
    @ResponseBody
//    parameter형식: ?name=hongildong&email=hongildong@naver.com
//    localhost:8080/hello/model-param1?name=hongildong
//    name의 값이 inputName으로 들어감
...
//    localhost:8080/author/1 or author?id=1  전자는 pathvariable 방식, 후자는 parameter 방식
...
//        사용자로부터 받아온 내용 출력
    ...
    @ResponseBody
    public String formPost2(@ModelAttribute Hello hello){ //ModelAttribute는 생략 가능(가독성을 위해 붙이는 것을 선호하기도 함)
//        사용자로부터 받아온 내용 출력
        System.out.println(hello);
        return "OK";
    }
...
//    form태그 name, email, password, file
    ...
    public String formfileHandle(Hello hello,
                                 @RequestParam(value="photo")MultipartFile photo){ // 아니면 Hello에 선언해놓으면 됨
        System.out.println(hello);
...
//        name, email, password를 전송
    ...
    @ResponseBody
//    axios를 통해 넘어오는 형식이 key1=value1&key2=value2 등 url인코딩 방식
...
//                                  formdata를 통해 json, file을 처리할 때 RequestPart 어노테이션을 많이 사용한다
                                  @RequestPart("hello") Hello hello,
                                  @RequestPart("file")MultipartFile file) // multipart-formdata 형식으로 받음
    {
        System.out.println(hello);
...
//        Hello hello = Hello.builder().name("").email("").build();
    }
}