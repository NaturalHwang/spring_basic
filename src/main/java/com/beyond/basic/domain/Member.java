...
@Entity // JPA부터 Entity를 반드시 붙여줘야 함.
//@AllArgsConstructor
@NoArgsConstructor  // 위를 쓰려면 아래는 항상 세트로 다녀야됨. 기본 생성자는 JPA에서 필수.
...
//    @Setter
//    @Column(name = "pw") 이렇게 할 수 있으나 컬럼명과 변수명은 일치시키는 것이 혼선을 줄일 수 있음
    private String password; // camel case로 작성 시 자동으로 _ 붙음(ex. passWord -> pass_word).
    // ∴  _ 넣고싶으면 pass_word 식으로 작성하는 것이 좋음(혼선이 생길 경우에만)
    @OneToMany(mappedBy = "member") // 필요에 의해서만 사용. ex) 글쓴이가 쓴 글의 수 같이 조회되게 하고 싶다.
...
//    일반적으로 의도를 명확하게 한 메서드를 별도로 만들어 사용하는 것을 권장.
    ...
    public MemberDetailResDto detFromEntity(){
        ...
        String time = createdTime.getYear() + "년 " + createdTime.getMonthValue() + "월 " + createdTime.getDayOfMonth() + "일";
        return new MemberDetailResDto(this.id, this.name, this.email, this.password, time);
    }
